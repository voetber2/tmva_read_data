...
int main(){                                                               
  //TFile *sig = TFile::Open("/msu/data/t3work4/luisonig/H1jets_ggf/NTuplesFiles/H1.0j_GGFHT_B_6500_pt25.0_eta4.5_r100_100.root");
  //TFile *sig_2 = TFile::Open("/msu/data/t3work4/luisonig/H1jets_ggf/NTuplesFiles/H1.0j_GGFHT_B_6500_pt25.0_eta4.5_r100_101.root");
  //TFile *bg = TFile::Open("/msu/data/t3work4/gosam_diphoton_jet/born/born6.root");                          
  //TFile *bg_2 = TFile::Open("/msu/data/t3work4/gosam_diphoton_jet/born/born5.root");                                
  //===============================================                       
  TFile* out = TFile::Open("data_17feat_pt200p.root", "RECREATE"); 
  //===============================================                         
  TChain chain_b("t3"); 
  TChain chain_s("t3"); 
  ...
  for (long j=0; j<entries_s; ++j){                                             
    ...
    select &= (jet.Rapidity()<4.4);       
      ...
      //Diphoton Pt cuts 
    //select &= (higgs.Pt()<50.);
    //select &= (higgs.Pt()>50. && higgs.Pt()<100.);
    //select &= (higgs.Pt()>100. && higgs.Pt()<150);
    //select &= (higgs.Pt()>150. && higgs.Pt()<200);
    select &= (higgs.Pt()>200.);    
  ...
  for (long k=0; k<entries_b; ++k){                                             
    ...
    //Diphoton Pt cuts 
    //select &= (diphoton.Pt()<50.);
    //select &= (diphoton.Pt()>50. && diphoton.Pt()<100.);
    //select &= (diphoton.Pt()>100. && diphoton.Pt()<150);
    //select &= (diphoton.Pt()>150. && diphoton.Pt()<200);
    select &= (diphoton.Pt()>200.);    
