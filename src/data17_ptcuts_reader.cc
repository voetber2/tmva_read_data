...
int main(){                                                               
  TFile *sig = TFile::Open("$TMPDIR/H1.0j_GGFHT_B_6500_pt25.0_eta4.5_r100_103.root");
  TFile *bg = TFile::Open("$TMPDIR/born1_1.root");                          
  
  TFile *data = TFile::Open("$TMPDIR/data15.root"); 
  //===============================================                       
  TFile* out = TFile::Open("data_17feat_pt200p_reader.root", "RECREATE"); 
  //===============================================                         
  TChain* chain_b = (TChain*)bg->Get("t3"); 
  TChain* chain_s = (TChain*)sig->Get("t3");                              
  
  TChain* chain_d = (TChain*)data->Get("mini");                              
  //===============================================                            
  ...
  for (long k=0; k<entries_d; ++k){
    ...
    //Diphoton Pt cuts 
    //select &= (yy.Pt()<50.);
    //select &= (yy.Pt()>50. && yy.Pt()<100.);
    //select &= (yy.Pt()>100. && yy.Pt()<150);
    //select &= (yy.Pt()>150. && yy.Pt()<200);
    select &= (yy.Pt()>200.);    
